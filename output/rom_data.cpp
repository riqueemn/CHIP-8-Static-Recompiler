...
int main() {
    ...
    { uint16_t res = V[6] + V[8]; V[15] = (res > 255); V[6] = res & 0xFF; }
    L25a: ; L25b: ;
    while (const std::optional event = window.pollEvent()) {
        if (event->is<sf::Event::Closed>()) { window.close(); return 0; }
    }
    { uint16_t res = V[7] + V[9]; V[15] = (res > 255); V[7] = res & 0xFF; }
    L25c: ; L25d: ;
    while (const std::optional event = window.pollEvent()) {
        if (event->is<sf::Event::Closed>()) { window.close(); return 0; }
    }
    ...
    { uint16_t res = V[14] + V[3]; V[15] = (res > 255); V[14] = res & 0xFF; }
    L2aa: ; L2ab: ;
    while (const std::optional event = window.pollEvent()) {
        if (event->is<sf::Event::Closed>()) { window.close(); return 0; }
    }
